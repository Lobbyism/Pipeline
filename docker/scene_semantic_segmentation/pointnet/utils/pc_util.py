...
def write_ply_color(points, labels, out_filename, num_classes=None):
    """ Color (N,3) points with labels (N) within range 0 ~ num_classes-1 as OBJ file """
    label_colors = [
        [200, 200, 200],  # 0: unannotated (light gray instead of black!)
        [255, 0, 0],  # 1: human (red)
        [0, 255, 0],  # 2: chair (green)
        [0, 0, 255],  # 3: floor (blue)
        [255, 255, 0],  # 4: table (yellow)
        [255, 165, 0],  # 5: desk (orange)
        [255, 0, 255],  # 6: bed (magenta)
        [0, 255, 255],  # 7: bookshelf (cyan)
        [128, 0, 128],  # 8: sofa (purple)
        [0, 128, 128],  # 9: sink (teal)
        [128, 128, 0],  # 10: bathtub (olive)
        [255, 192, 203],  # 11: toilet (pink)
        [192, 192, 192],  # 12: curtain (silver)
        [128, 0, 0],  # 13: counter (maroon)
        [0, 128, 0],  # 14: door (dark green)
        [0, 0, 128],  # 15: window (navy)
        [210, 105, 30],  # 16: shower curtain (chocolate)
        [139, 69, 19],  # 17: refrigerator (brown)
        [70, 130, 180],  # 18: picture (steel blue)
        [154, 205, 50],  # 19: cabinet (yellow green)
        [255, 20, 147],  # 20: otherfurniture (deep pink)
    ]
    labels = labels.astype(int)
    N = points.shape[0]
    fout = open(out_filename, 'w')
    for i in range(N):
        label = labels[i] % len(label_colors)  # in case label exceeds defined colors
