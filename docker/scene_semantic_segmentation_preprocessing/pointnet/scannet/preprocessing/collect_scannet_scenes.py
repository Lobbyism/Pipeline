...
SCANNET_DIR = os.path.join(BASE_DIR, 'preprocessed_scenes')
SCENE_NAMES = [line.rstrip() for line in open(os.path.join(BASE_DIR, 'scenes_all.txt'))]
...
def collect_one_scene_data_label(scene_name, out_filename):
    ...
    mesh_seg_filename = os.path.join(data_folder, '%s.segs.json' % (scene_name))
    ...
    # Raw points in XYZ
    ply_filename = os.path.join(data_folder, '%s.ply' % (scene_name))
    points = pc_util.read_ply_xyz(ply_filename)
...
if __name__ == '__main__':
    output_folder = os.path.join(BASE_DIR, 'npy_scenes')
